...
import { Heart, Minus, Plus, Truck, PackageCheck, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="container-page py-8">
      <div className="flex items-center gap-1 text-xs text-neutral-500 mb-6">
        ...
        <Link to={`/shop?category=${product.category}`} className="hover:text-brand">{categoryName}</Link>
        <ChevronRight size={14} />
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            {product.oldPrice > product.price && (
              <>
                <span className="text-neutral-400 line-through">${product.oldPrice.toFixed(2)}</span>
          ...
          <p className="text-neutral-600 mb-5 leading-relaxed">{product.description}</p>
          ...
          <p className={`text-sm mb-5 font-medium ${product.stock > 10 ? 'text-green-600' : 'text-amber-600'}`}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <button onClick={handleAddToCart} className="btn-outline flex-1" disabled={product.stock === 0}>
              {added ? 'Added to Cart ✓' : 'Add to Cart'}
            </button>
            <button onClick={handleBuyNow} className="btn-primary flex-1" disabled={product.stock === 0}>
              Buy Now
            </button>
          </div>
          ...
          <div className="border-t border-neutral-200 pt-4 space-y-2 text-sm text-neutral-500">
            <p className="flex items-center gap-2"><Truck size={16} /> Free Shipping on orders over $100</p>
            <p className="flex items-center gap-2"><PackageCheck size={16} /> Estimated delivery: 3-5 business days</p>
          </div>
        </div>
      </div>
