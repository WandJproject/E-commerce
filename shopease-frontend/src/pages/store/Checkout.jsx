...
export default function Checkout() {
  ...
  const validate = () => {
    ...
    if (!/^\d{13,19}$/.test(form.cardNumber.replace(/\s/g, ''))) errs.cardNumber = 'Enter a valid card number'
  ...
  if (items.length === 0) {
    return (
      <div className="container-page py-24 text-center">
        ...
        <p className="text-neutral-500">Your cart is empty. Add products before checking out.</p>
      </div>
    )
  }
          ...
          <div className="card p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input name="expiry" value={form.expiry} onChange={handleChange} placeholder="08/28" className="input-field" />
                {errors.expiry && <p className="text-xs text-red-500 mt-1">{errors.expiry}</p>}
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">CVV</label>
                <input name="cvv" value={form.cvv} onChange={handleChange} placeholder="123" className="input-field" />
                {errors.cvv && <p className="text-xs text-red-500 mt-1">{errors.cvv}</p>}
              </div>
            </div>
            <p className="text-xs text-neutral-400 mt-3">This is a demo checkout. No real payment is processed.</p>
          </div>
        </div>
